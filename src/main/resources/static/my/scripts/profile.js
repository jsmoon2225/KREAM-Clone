...
$ContactForm.onsubmit = (e) => {
    ...
    xhr.onreadystatechange = () => {
        ...
        if (response['result'] === 'success') {
            ...
            $ContactForm.hide();
            const $contact = $contactdiv.querySelector(':scope > .unit_content > .contact');
            $contact.innerText = response['contact'];
        }
...
$Modify.onsubmit = (e) => {
    ...
    xhr.onreadystatechange = () => {
        ...
        const [title, content, onclick] = {
            ...
            failure_password_incorrect: ['비밀번호 재설정', '비밀번호를 재설정할 수 없습니다. 비밀번호를 규격에 맞게 입력해 주세요', ($dialog) => Dialog.hide($dialog)],
            failure_duplicate_password: ['비밀번호 재설정', '비밀번호를 재설정할 수 없습니다. 이전 비밀번호가 일치하지 않습니다.', ($dialog) => Dialog.hide($dialog)],
            success: ['비밀번호 재설정', '비밀번호를 성공적으로 재설정하였습니다. 확인 버튼을 클릭하면 로그인 페이지로 이동합니다.', ($dialog) => {
                Dialog.hide($dialog);
